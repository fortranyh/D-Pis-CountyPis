...
namespace PIS_Sys
{
    public partial class Frm_TempletImport : DevComponents.DotNetBar.Office2007Form
    {
        public Frm_TempletImport()
        {
            InitializeComponent();
        }
        ...
        private void BuildTree(string Usercode, string userName, TreeView tvwTemplete, Boolean flag)
        {
            ...
            DataTable dt = ins.GetTreeDtms_Templet(Usercode);
            if (dt != null && dt.Rows.Count == 0)
            {
                //插入根节点
                string uid = Helper.GetUidStr();
                ins.InsertDTtempletRoot(Usercode, uid);
                dt = ins.GetTreeDtms_Templet(Usercode);
            }
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        private void BuildCopySubTree(String sourceid, String desid, String sourceysbh, String desysbh)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                if (dt.Rows[0]["ParentID"].ToString() != "-1")
                {
                    ...
                    if (icount == 0)
                    {
                        DBHelper.Model.dtms_templet insmodel = new DBHelper.Model.dtms_templet();
                    ...
                    dt = ins.GetChildTreeDtms_Templet(sourceysbh, sourceid);
