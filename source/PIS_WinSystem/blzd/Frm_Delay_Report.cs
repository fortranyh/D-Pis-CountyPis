...
{
    public partial class Frm_Delay_Report : DevComponents.DotNetBar.Office2007Form
    {
        public string BLH = "";
        public string exam_no = "";
        public Frm_Delay_Report()
        {
            InitializeComponent();
        }
        private string patient_name, sex, age, req_dept, patient_source;
        private void Frm_Delay_Report_Load(object sender, EventArgs e)
        {
            ...
            this.comboBoxEx3.SelectedIndexChanged += new System.EventHandler(this.comboBox1_SelectedIndexChanged);
        }
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!comboBoxEx3.Text.Equals(""))
            {
                richTextBoxEx1.Text = comboBoxEx3.Text;
            }
        }
        //保存延时报告
        private void buttonX2_Click(object sender, EventArgs e)
        {
            ...
            {
                DBHelper.BLL.exam_delay_report ins = new DBHelper.BLL.exam_delay_report();
                DBHelper.Model.exam_delay_report insM = new DBHelper.Model.exam_delay_report();
                ...
                if (dt != null && dt.Rows.Count == 1)
                {
                    if (dt.Rows[0]["sh_flag"].ToString().Equals("1"))
                    {
                        if (ins.ShReport(BLH, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), 1, comboBoxEx2.SelectedValue.ToString(), comboBoxEx2.Text.Trim()))
                        {
                            ...
                            Frm_TJInfo("提示", "发送成功，临床已可见!");

                            //获取检查状态
                            DBHelper.BLL.exam_master insStatus = new DBHelper.BLL.exam_master();
                            int status = insStatus.GetStudyExam_Status(BLH);
                            if (status > 30 && status < 40)
                            {
                                //更新报告状态
                                insStatus.UpdateExam_Status(BLH, "36");
                                //更新延时原因到主表
                                insStatus.UpdateDelay_reason(BLH, richTextBoxEx1.Text.Trim());
                            }
                            buttonX3.TextColor = Color.Blue;
                        }
        ...
        private void Frm_Delay_Report_FormClosed(object sender, FormClosedEventArgs e)
        {
            this.comboBoxEx3.SelectedIndexChanged -= new System.EventHandler(this.comboBox1_SelectedIndexChanged);
        ...
        private void buttonX4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string dURL = "http://192.168.8.142:8099/BLReport.ashx?exam_no=" + exam_no;
                ps.StartInfo.FileName = "iexplore.exe";
                ps.StartInfo.Arguments = dURL;
                ps.Start();
            }
            catch
            {
