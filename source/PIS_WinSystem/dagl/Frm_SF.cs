...
{
    public partial class Frm_SF : DevComponents.DotNetBar.Office2007Form
    {
        public Frm_SF()
        {
            InitializeComponent();
        ...
        private void Frm_SF_Load(object sender, EventArgs e)
        {
            //高度自动适应
            ...
            try
            {
                if (superGridControl1.PrimaryGrid.VirtualRowCount > 0)
                {
                    ...
                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        wk.Worksheets[0].Name = sj + "随访信息列表";
                        DataTable dt = superGridControl1.PrimaryGrid.DataSource as DataTable;
                        AddExcelSHeet(wk, dt);
                        wk.Save(localFilePath);
                        MessageBox.Show("导出成功：" + localFilePath, "随访数据导出", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            {
                sb.AppendFormat("  study_no = '{0}'", Txtblh.Text.Trim().Replace("'", "").ToUpper());
            }
            else
            {
                sb.AppendFormat("  create_dt>='{0} 00:00:00' and create_dt<='{1} 23:59:59'", dtStart.Value.ToString("yyyy-MM-dd"), dtEnd.Value.ToString("yyyy-MM-dd"));
            }
            DataTable dt = new DataTable();
            DBHelper.BLL.pat_zzsf_advice ins = new DBHelper.BLL.pat_zzsf_advice();
            dt = ins.GetdtAdvice(sb.ToString());
