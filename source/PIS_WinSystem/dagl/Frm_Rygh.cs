...
{
    public partial class Frm_Rygh : DevComponents.DotNetBar.Office2007Form
    {
        public Frm_Rygh()
        {
            InitializeComponent();
        ...
        private void Frm_Rygh_Load(object sender, EventArgs e)
        {
            //高度自动适应
        ...
        private void RefreshData()
        {
            ...
            sb.AppendFormat(" ghrq>='{0} 00:00:00' and ghrq<='{1} 23:59:59'", dateTimePicker1.Value.ToString("yyyy-MM-dd"), dtEnd.Value.ToString("yyyy-MM-dd"));
            ...
            dt = ins.GetInfoRygh(sb.ToString());
        ...
        {
            Frm_hcgl_rygh ins = new Frm_hcgl_rygh();
            ins.BringToFront();
        ...
        private void superGridControl1_SelectionChanged_1(object sender, GridEventArgs e)
        {
            if (superGridControl1.PrimaryGrid.VirtualRowCount > 0)
            {
                ...
                Oldid = id;
                textBox8.Text = Row.Cells["wp"].Value.ToString();
                textBox6.Text = Row.Cells["jl"].Value.ToString();
                textBox7.Text = Row.Cells["qm"].Value.ToString();
                textBox5.Text = Row.Cells["bz"].Value.ToString();
                dateTimePicker2.Value = Convert.ToDateTime(Row.Cells["ghrq"].Value.ToString());
            }
        }
        private void buttonX5_Click(object sender, EventArgs e)
        {
            if (textBox8.Text.Trim().Equals("") || textBox6.Text.Trim().Equals("") || textBox7.Text.Trim().Equals(""))
            {
            ...
            if (superGridControl1.PrimaryGrid.VirtualRowCount > 0)
            {
                ...
                id = Row.Cells["id"].Value.ToString();
                DBHelper.BLL.hcgl ins = new DBHelper.BLL.hcgl();
                if (ins.UpdateInfoRygh(textBox8.Text.Trim(), textBox6.Text.Trim(), dateTimePicker2.Value.ToString("yyyy-MM-dd HH:mm:ss"), textBox7.Text.Trim(), textBox5.Text.Trim(), id) == 1)
                {
                    Frm_TJInfo("提示", "更新成功！");
                    RefreshData();
                }
