...
namespace DBhleper.BLL
{
   ...
   public class exam_delay_report
    {
        //获取延时原因
        public DataTable GetYsyyData()
        {

            string sqlstr = "select delay_ys,id from exam_ysbg_dict";
            DataTable dt = null;
            try
            {
            ...
            {
                DBProcess.ShowException(ex, "GetYsyyData 执行语句异常：" + sqlstr);
            }
            return dt;
        }

       public Boolean SaveReport(Model.exam_delay_report insM)
       {
           Boolean Zx_Result = false;
           string sqlstr = "select count(*) as sl from exam_delay_report where study_no=@study_no";
           try
           {
               DbCommand cmd = DBProcess._db.GetSqlStringCommand(sqlstr);
               ...
               {
                   //插入
                   sqlstr = "insert into exam_delay_report(study_no,content,report_doc_name,report_doc_code,report_datetime) values(@study_no,@content,@report_doc_name,@report_doc_code,@report_datetime)";
               ...
               else if (sl == 1)
               {
                       ...
                       //更新
                   sqlstr = "update exam_delay_report set  content=@content,report_doc_name=@report_doc_name,report_doc_code=@report_doc_code,report_datetime=@report_datetime where study_no=@study_no";
                       cmd = DBProcess._db.GetSqlStringCommand(sqlstr);
                       DBProcess._db.AddInParameter(cmd, "@study_no", DbType.String, insM.study_no);
                       DBProcess._db.AddInParameter(cmd, "@content", DbType.String, insM.content);
                       DBProcess._db.AddInParameter(cmd, "@report_doc_name", DbType.String, insM.report_doc_name);
       ...
       public DataTable GetData(string study_no)
       {
           ...
           string sqlstr = "select study_no,content,report_doc_name,report_doc_code,sh_doc_name,sh_doc_code,sh_flag,date_format(sh_datetime,'%Y-%m-%d %H:%i:%s') as sh_datetime,date_format(report_datetime,'%Y-%m-%d %H:%i:%s') as report_datetime from exam_delay_report where study_no=@study_no";
           DataTable dt = null;
           try
           {
               DbCommand cmd = DBProcess._db.GetSqlStringCommand(sqlstr);
       ...
       {
           string sqlstr = "select study_no,content,report_doc_name,sh_doc_name, date_format(sh_datetime,'%Y-%m-%d %H:%i:%s') AS    sh_datetime, date_format(report_datetime,'%Y-%m-%d %H:%i:%s') AS report_datetime from exam_delay_report where 1=1 " + tj;
           DataTable dt = null;
           try
           {
               DbCommand cmd = DBProcess._db.GetSqlStringCommand(sqlstr);
       ...
       public Boolean ShReport(string study_no,string sh_datetime, int sh_flag, string sh_doc_code, string sh_doc_name)
       {
           ...
           string sqlstr = "update exam_delay_report set  sh_doc_name=@sh_doc_name,sh_doc_code=@sh_doc_code,sh_flag=@sh_flag,sh_datetime=@sh_datetime where study_no=@study_no";
