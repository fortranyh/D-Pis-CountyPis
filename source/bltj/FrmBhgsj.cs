...
{
    public partial class FrmBhgsj : DevComponents.DotNetBar.Office2007Form
    {
        public FrmBhgsj()
        {
            InitializeComponent();
        ...
        private void FrmBhgsj_Load(object sender, EventArgs e)
        {
            //高度自动适应
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            sb.AppendFormat(" note_datetime>='{0} 00:00:00' and note_datetime<='{1} 23:59:59'", dtStart.Value.ToString("yyyy-MM-dd"), dtEnd.Value.ToString("yyyy-MM-dd"));

            //加载已经存在的不合格
            DataSet _DataSet = new DataSet();
            DBHelper.BLL.exam_specimens_qualified ins = new DBHelper.BLL.exam_specimens_qualified();
            _DataSet = ins.GetDsSpecimens_qualifiedInfo(sb.ToString());
            sb.Clear();
            if (_DataSet != null && _DataSet.Tables[0].Rows.Count > 0)
            {
                ...
                superGridControl1.PrimaryGrid.Footer.Text = String.Format("共查询到不合格送检总数为(<font color='blue'><b>{0}</b></font>)", _DataSet.Tables[0].Rows.Count.ToString());
            }
            else
            {
                ...
                superGridControl1.PrimaryGrid.Footer.Text = String.Format("共查询到不合格送检总数为(<font color='blue'><b>{0}</b></font>)", "0");
            }
        }
            ...
            try
            {
                if (superGridControl1.PrimaryGrid.VirtualRowCount > 0)
                {
                    ...
                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        wk.Worksheets[0].Name = sj + "病理不合格送检信息";
                        DataTable dt = superGridControl1.PrimaryGrid.DataSource as DataTable;
                        AddExcelSHeet(wk, dt);
                        wk.Save(localFilePath);
                        MessageBox.Show("导出成功：" + localFilePath, "病理不合格送检数据导出", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception ex)
